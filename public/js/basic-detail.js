...
function changeStreamServer(phimId, serverNumber) { $('#linkhongId').hide();
    svid = 'db_' + serverNumber;
    $.ajax({ beforeSend: function() { $('#loading_logo_container').show() }, type: 'GET', url: '/change-sever/' + phimId + '/' + serverNumber, success: function(data) { $('#loading_logo_container').hide();
            ...
            $('#phimContainId').attr('src', data.newUrl); if (data.isErrorUrl) { $('#linkhongId').show();
                $('#alertContentId')[0].textContent = "     Server " + serverNumber + " hỏng    " } else { $("#phimContainId")[0].play() } }, }) }
